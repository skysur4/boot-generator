                        ...
                        while (schemas.next()) {
                            ...
                            try (ResultSet tables = metaData.getTables(null, schemaName, projectConfig.getTableFilter(), new String[]{"TABLE"})) {
                                ...
                                while (tables.next()) {
                                    ...
                                    List<GenExportedKey> exportedKeyList = Lists.newArrayList();
                                     ...
                                     */
                                    try (ResultSet exportedKeys = metaData.getExportedKeys(null, schemaName, tableName)) {
                                        while (exportedKeys.next()) {
                                            GenExportedKey.GenExportedKeyBuilder exportedKey = GenExportedKey.builder();

                                            exportedKey.table(exportedKeys.getString("PKTABLE_NAME"))
                                                    .column(exportedKeys.getString("PKCOLUMN_NAME"))
                                                    .subTable(exportedKeys.getString("FKTABLE_NAME"))
                                                    .subColumn(exportedKeys.getString("FKCOLUMN_NAME"))
                                                    .seq(exportedKeys.getShort("KEY_SEQ"));

                                            exportedKeyList.add(exportedKey.build());
                                        }
                                    }
                                    table.exportedKeys(exportedKeyList);
